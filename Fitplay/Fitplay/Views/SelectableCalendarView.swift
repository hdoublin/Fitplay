...
//
//  Created by Scorpus on 04/19/23
//
...
public struct SelectableCalendarView: View {
    ...
    public init(monthToDisplay: Date, dateSelected: Binding<Date>, allowSwitchMonth: Bool = true, showMonthLabel: Bool = true, isDateCircleFilled: ((Date) -> Bool)? = nil,selectedDates:Binding<[Date]>) {
                        ...
                        if Calendar.current.isDate(date, equalTo: monthToDisplay, toGranularity: .month) {
                            if let isDateCircleFilled = isDateCircleFilled {
                                Text("\(date.getDayNumber())")
                                    .foregroundColor(self.isContain(comparingTo: date) ? .white : .black)
                                    .font(.system(size: 15))
                                    .id(date)
                                    .addCircularBackground(isFilled: self.isContain(comparingTo: date), isSelected: dateSelected.isSameDay(comparingTo: date))
                                    .onTapGesture {
    ...
    func isContain(comparingTo: Date) -> Bool{
        if let first = self.selectedDates.first(where: { date in
            comparingTo.isSameDay(comparingTo: date)
        }){
            return true
        }
        return false
